...
const ResourcesPage = ({resources, setResources, showSOS, setShowSOS}) => {
		...
		if (item && type && location && date && parsedQuantity > 0) {
			...
			try {
				...
				setResources([
					newEntry,
					...resources
				]); // optionally fetch from Firebase after
				setNewResource({
					item: '',
					type: '',
					location: '',
	...
	useEffect(() => {
		...
		const unsubscribeSOS = onSnapshot(collection(db, "sosRequests"), (snapshot) => {
			setSosRequests(snapshot.docs.map(doc => ({
				id: doc.id,
				...doc.data()
			})));
		});
	...
	const [sosForm, setSosForm] = useState({type: 'Pads Emergency', location: '', description: ''});
	...
	return (
		<div className="resources-page">
			<div className="header-row">
				...
				<div>
					...
					<button onClick={
						...
						className="sos-button">🆘 SOS Request</button>
				</div>
			</div>
					...
					<div className="add-resource-form">
						...
						<div className="sos-form-buttons">
							<button onClick={handleAddResource}
								className="sos-send-button">Add Resource</button>
							<button onClick={
									() => setShowAddResourceForm(false)
								}
				...
				<div className="sos-form-container">
					...
					<div className="sos-form">
						...
						<input type="text" placeholder="Your Location"
							...
							className="input-text"/>
						<textarea placeholder="Brief description of urgent need..."
							value={
								sosForm.description
							}
							onChange={
						...
						<div className="sos-form-buttons">
							<button onClick={handleSubmitSOS}
								className="sos-send-button">🆘 Send SOS Alert</button>
							<button onClick={
									() => setShowSOS(false)
								}
								className="cancel-button">Cancel</button>
			...
			<div className="category-filters">
				...
				<button className={
						`category-button ${
							selectedCategory === 'Pads Emergency' ? 'active' : ''
						}`
					}
					onClick={
						() => setSelectedCategory('Pads Emergency')
